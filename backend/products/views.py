...
from .models import Category, Product, Product_Seller, Sellers
...
def getProductSellers(request):
    product_id = request.query_params['product_id']
    sellers_from_mtm = Product_Seller.objects.filter(product_id=product_id)
    seller_ids = []
    for seller in sellers_from_mtm:
        seller_ids.append(seller.seller_id)
    sellers = Sellers.objects.filter(seller_id__in = seller_ids)
